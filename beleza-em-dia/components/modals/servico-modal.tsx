...
export const SERVICE_CATEGORIES = ['Unhas', 'Cabelos', 'Cílios', 'Depilação', 'Maquiagem']

interface ServicoModalProps {
  isOpen: boolean
  service?: MockService | null
  onClose: () => void
  onSave: (data: Pick<MockService, 'name' | 'category' | 'price' | 'duration' | 'active' | 'description'>) => void
}

export function ServicoModal({ isOpen, service, onClose, onSave }: ServicoModalProps) {
  const [category, setCategory] = useState('Unhas')
  ...
  useEffect(() => {
    if (!isOpen) return
    const isKnownCategory = service?.category && SERVICE_CATEGORIES.includes(service.category)
    setCategory(isKnownCategory ? service!.category : service?.category ? 'Outra' : 'Unhas')
    setCustomCategory(isKnownCategory ? '' : service?.category || '')
  ...
  const save = (event: React.FormEvent) => {
    ...
    setError('')
    onSave({ name: name.trim(), category: customCategory.trim() || category.trim(), price: numericPrice, duration: numericDuration, active, description: description.trim() || undefined })
  }
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <form onSubmit={save} onClick={(event) => event.stopPropagation()} className="w-full max-w-md space-y-4 rounded-2xl bg-white p-5 shadow-2xl dark:bg-gray-900">
        ...
        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300">Categoria
          <select value={category} onChange={(event) => { setCategory(event.target.value); setCustomCategory(''); setName('') }} className="mt-1 w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800">
        ...
        <textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Descrição/Observação (opcional)" rows={3} className="w-full resize-none rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800" />
